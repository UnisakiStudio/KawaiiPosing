...
- アバターによって姿勢が崩れることがあります。アニメーション調整機能を使って修正してください
...
- いくつかのアバター向けのアニメーション調整が行われたプリセットファイル
- ゆにさきスタジオの別のツールや他ショップの商品を併用するための「他商品連携用Prefabs」

# 使い方
- 同封の「可愛いポーズツール.unitypackage」をダブルクリックするかUnityでインポートしてください
- （一度インポートすれば次からはALCOMなどから「可愛いポーズツール」をプロジェクトに追加することができます）
...
# プレビルドについて
可愛いポーズツールは、NDMFの機能を使用してアバタービルド時にメニューやアニメーションを生成していますが、
アバターをビルドするたびに処理を実行するとアバタービルドに時間がかかるようになってしまうため、
Ver3.0.0からプレビルド機能を追加いたしました。プレビルドを行うと可愛いポーズツールのデータがModularAvatarのコンポーネントに変換されます。
アニメーションの設定などを変更した場合は再度プレビルドしないと、アバタービルド時に毎回処理が実行されてしまうので、こまめにプレビルドを行ってください。

# アニメーション調整機能について
同封のアニメーションは桔梗を使用して作成しているため、一部のアバターでは想定通りの姿勢にならないことがあります。
そのため、Ver3.0.0以降では姿勢のプレビューと調整を行う機能を導入いたしました。
可愛いポーズオブジェクトのインスペクターより「アニメーション調整」ボタンを押すと、
シーンに配置してあるアバターをプレビューしながら

# プリセットについて
可愛いポーズツールVer3.0.0以降ではアバターに合わせて姿勢を調整するアニメーション調整機能が追加されています。
この機能を使って作成した調整アニメーションファイル、およびそれを設定した状態で作成したプリセットについては、
ゆにさきスタジオに許諾を得ずご自由に配布、販売することができます。
ご自身の自作アバター配付時や、ツールに同封していないアバターのプリセットなど、是非共有してください。
